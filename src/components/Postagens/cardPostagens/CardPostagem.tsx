...
function CardPostagem({ post }: CardPostagemProps) {
  ...
  const curtir = () => {
    if (curtidas == 0) {
      setCurtidas(curtida => {
        const contarCurtidas = Number(curtida) + 1;
        localStorage.setItem(`curtidas_${post.id}`, String(contarCurtidas));
        return contarCurtidas;
      });
    } else if (curtidas != 0) {
      setCurtidas(curtida => {
        const contarCurtidas = Number(curtida) - 1;
  ...
  useEffect(() => {
    const initialValue = localStorage.getItem(`curtidas_${post.id}`);
    if (initialValue) setCurtidas(Number(initialValue));
  }, [post.id]);
